 ...
 * PenisQueue
 * %%
 * Copyright (C) 2021 Christallinqq
 * %%
 ...
 * #L%
 */
package me.christallinqq.penisqueue.bungee.commands;
...
import me.christallinqq.penisqueue.bungee.utils.StorageTool;
...
public final class MainCommand extends Command implements TabExecutor {
    ...
    public MainCommand(PenisQueue plugin) {
        super("penisqueue", null, "pq");
        this.plugin = plugin;
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length > 0) {
            switch (args[0].toLowerCase()) {
                case "version":
                    sendLine(sender);
                    sender.sendMessage(new ComponentBuilder("PenisQueue").color(ChatColor.GOLD).create());
    ...
    private void sendBanHelp(CommandSender sender) {
        ...
        sender.sendMessage(new ComponentBuilder("/pq shadowban Penismaster 2d").color(ChatColor.GOLD).create());
        sendLine(sender);
    }
    ...
    private void sendUnBanHelp(CommandSender sender) {
        ...
        sender.sendMessage(new ComponentBuilder("Example:").color(ChatColor.GOLD).create());
        sender.sendMessage(new ComponentBuilder("/pq unshadowban Penismaster").color(ChatColor.GOLD).create());
        sendLine(sender);
    }
