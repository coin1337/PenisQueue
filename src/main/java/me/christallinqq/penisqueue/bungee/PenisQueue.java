...
import me.christallinqq.penisqueue.bungee.hooks.PenisMOTDPlaceholder;
...
import me.christallinqq.penisqueue.bungee.commands.MainCommand;
import me.christallinqq.penisqueue.bungee.listeners.PenisListener;
import me.christallinqq.penisqueue.bungee.listeners.QueueListener;
...
public final class PenisQueue extends Plugin {
    @Getter
    ...
    public void onEnable() {
        ...
        if (getProxy().getPluginManager().getPlugin("PenisMOTD") != null) {
            logger.info(ChatColor.BLUE + "Hooking into PenisMOTD");
            new PenisMOTDPlaceholder();
        }

        logger.info(ChatColor.BLUE + "Registering plugin messaging channel");
        getProxy().registerChannel("penis:queue");
        ...
        manager.registerListener(this, new PenisListener(this));
        ...
        new UpdateChecker(this, 83541).getVersion(version -> {
            if (this.getDescription().getVersion().equalsIgnoreCase(version)) {
                ...
                logger.info(ChatColor.RED + "Download it at: https://github.com/christallinqq/penisqueue");
            }
        });
    ...
    private void loadConfig() throws IOException {
        ...
        Arrays.asList(Config.class.getDeclaredFields()).forEach(it -> {
            try {
                ...
                for (String str : text) {
                    if (str.toLowerCase().startsWith(PenisQueue.class.getPackage().getName().toLowerCase())) {
                        value = str;
                    }
                }
    ...
    private void sendCustomData() {
        ...
        networkPlayers.forEach(player -> {
            if (player.getServer() != null)
                player.getServer().getInfo().sendData("penis:queue", out.toByteArray());
        });
    }
}
